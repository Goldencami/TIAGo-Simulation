...
class LiftArmControl : public rclcpp::Node {
public:
    LiftArmControl(): Node("lift_arm_control"), arm_moved_(false) {
        service_ = this->create_service<std_srvs::srv::Trigger>(
            "lift_arm",
            std::bind(&LiftArmControl::handleLiftArmRequest, this, 
                std::placeholders::_1, std::placeholders::_2)
        );

        RCLCPP_INFO(this->get_logger(), "LiftArmControl node initialized as service server.");
    }
...
private:
    ...
    void handleLiftArmRequest(
        ...
        if (!arm_moved_) {
            RCLCPP_INFO(this->get_logger(), "Received lift_arm request. Moving arm...");
            bool success = moveArmToGoal();

            response->success = success;
            response->message = success ? "Arm lifted successfully!" : "Arm failed to lift.";
    ...
    bool moveArmToGoal() {
        ...
        joint_goal["arm_2_joint"] = deg2rad(58.0);
        joint_goal["arm_3_joint"] = deg2rad(-81.0);
        joint_goal["arm_4_joint"] = deg2rad(94.0);
        joint_goal["arm_5_joint"] = deg2rad(57.0);
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<LiftArmControl>();
